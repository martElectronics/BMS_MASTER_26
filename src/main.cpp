...
// ============================================================================
//  UMBRALES Y TIEMPOS → include/bms_timing.h (compartido con el banco de
//  inyección de fallos, env `fault_bench`). Ahí están CELL_*, FAULT_*,
//  SAMPLE_*, COMM_*, WDG_TIMEOUT_US, PRECHARGE_TIMEOUT_MS y SDC_FILTER_MS.
// ============================================================================
#include "bms_timing.h"
#include "fault_inject.h"   // ganchos del banco (no-ops sin -D BMS_FAULT_INJECT)
...
// Las cadencias/ventanas viven en include/bms_timing.h (BMS_REINIT_RETRY_MS,
// BOOT_REINIT_ATTEMPTS): única fuente de verdad compartida con el banco.
...
// conserva la constante como 0 para no romper los bits de telemetría que la
// referenciaban (ID 15 B3 bit4, ID 17 B0 bit4), que ahora van siempre a 0.
...
void setup()
{
    ...
    // Salidas en estado seguro: SDC_TSON abierto. PRECHARGE_FAIL se pone LOW
    // aquí y NO se vuelve a escribir (lógica de precarga-fallida retirada).
    ...
    fiBegin();   // banco de inyección (no-op en producción)
...
void sampleAndEvaluate()
{
    ...
    if ((now - tV) >= SAMPLE_V_MS) {
        tV = now;
        lastResV = fiRead(bms.readVoltages(), 0);
    ...
    if ((now - tT) >= SAMPLE_T_MS) {
        tT = now;
        lastResT = fiRead(bms.readTemperatures(), 1);
        ...
        if (lastResT == BQResult::OK) {
            ...
            fT.sample(fiBad(badT, FI_T), now);
            // Pérdida de medida térmica (EV5.8.13): NTC abierto/inválido.
            fNtc.sample(fiBad(bms.hasOpenNtc(), FI_NTC), now);
        }
    }
    ...
    bool escalate = fiPrecharge(prechargeRunning)
                        ? readErr
    ...
    if (escalate && (now - tLastReinit) >= COMM_REINIT_RETRY_MS) {
        ...
        unsigned long tReinit0 = millis();       // banco: mide el bloqueo real
#endif
        bool okReinit = bms.reInit();
        IWatchdog.reload();
#ifdef BMS_FAULT_INJECT
        fiReinit(tReinit0, millis(), okReinit);
#endif
        bms.setMaxAttempts(BQ_AUTOADDR_ATTEMPTS);   // restaurar default
        if (okReinit) {
            bmsInitOk = true;
...
void updateBmsOk()
{
    ...
    // Banco de inyección: una foto del estado por loop. Bajo #ifdef porque el
    // struct + las asignaciones no siempre los elimina el optimizador, y este
    // fichero tiene que compilar a un binario IDÉNTICO en producción.
#ifdef BMS_FAULT_INJECT
    {
        FiState st{};
        st.fault[FI_V]        = faultV;
        st.fault[FI_T]        = faultT;
        st.fault[FI_NTC]      = faultNtc;
        st.fault[FI_COMM]     = faultComm;
        st.fault[FI_INIT]     = faultInit;
        st.fault[FI_HALL]     = faultHall;
        st.fault[FI_BMSFAULT] = bmsFault;
        st.commBadRun      = fComm.badRun;
        st.commBadMs       = fComm.badRun ? (now - fComm.tStart) : 0;
        st.bmsInitOk       = bmsInitOk;
        st.prechargeRunning= prechargeRunning;
        fiObserve(now, st);
    }
#endif
...
void updateTson()
{
    ...
    // en setup y no se vuelve a escribir). El timeout solo AVISA por serial y
    // para el temporizador; no enclava nada ni acciona el pin.
    // prechargeRunning se mantiene porque lo consume sampleAndEvaluate() para
    // reconectar el BQ de forma agresiva mientras dura la precarga.
    ...
    if (sdcTson && !sdcTsonPrev) {               // flanco de armado
        ...
        if (digitalRead(PIN_PRECHARGE_DONE))
            Serial.println(F("[PRE] aviso: PRECHARGE_DONE ya venía HIGH al armar."));
    ...
    if (prechargeRunning) {
        if (digitalRead(PIN_PRECHARGE_DONE)) {
            ...
            Serial.println(F("[PRE] aviso: PRECHARGE_DONE no llegó a tiempo."));
        }
    }
}
...
void handleSerial()
{
    ...
    default:
        fiCommand(cmd);   // comandos del banco (no-op en producción)
        break;
    }
}
